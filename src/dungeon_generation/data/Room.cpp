...
bool Room::CloseEnoughX(const Room& other)
{
    ...
    if (x > other.x)
    {
        return (other.x + other.width) - (x - width) > threshold;
    }
    else
    {
        return (x - width) - (other.x - other.width) > threshold;
    }
}
...
bool Room::CloseEnoughY(const Room& other)
{
    ...
    if (y > other.y)
    {
        return (other.y - other.height) - (y - height) > threshold;
    }
    else
    {
        return (y - height) - (other.y - other.height) > threshold;
    }
}
